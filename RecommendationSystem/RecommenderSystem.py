...
def print_summary(user_name, predictions_df, movies_number=5):
    ...
    reviewed = get_user_reviews(user_index)
    best = get_best_reviews_with_genres(reviewed, movies_number)
    recommended = generate_recommendations_with_genres(
        user_index, predictions_df, reviewed, movies_number)
...
def generate_recommendations_with_genres(user_index, predictions_df,
                                         user_reviewed_movies, movies_number=5):
    user_prediction_scores = predictions_df.iloc[user_index]\
        .sort_values(ascending=False)

    user_prediction_scores = user_prediction_scores.drop(user_reviewed_movies.index)
...
def get_best_reviews_with_genres(user_reviewed_movies, movies_number=5):
...
for name in user_names:
    print_summary(name, predictions_df)
